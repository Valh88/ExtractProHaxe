...
class SceneManager
{
	...
	public function new(app : hxd.App, s2d : Scene2D, style : Style, bus : EventBus, gd : GameData)
	{
		this.app = app;
		this.s2d = s2d;
		this.style = style;
	...
	function createScene(id : GameScene) : Scene3D
	{
		return switch (id)
		{
			case Lobby: new LobbyView(s2d, style, gd, bus);
			case Gameplay: new GamePlayView(s2d, style, gd);
		}
	}
